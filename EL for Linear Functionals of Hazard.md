...
# 2. Cox 模型中的经验似然分析
同样地，设 $X_1, \ldots, X_n$ 表示一组独立的病人生存时间随机变量， $z_1, \ldots, z_n$ 则为对应的协变量向量，例如患者的性别、年龄或治疗方案等。

Cox 比例风险回归模型规定，第 $i$ 个个体的寿命 $X_i$ 所对应的风险函数 $\lambda_i(t)$，与其协变量 $z_i$ 之间满足以下关系：
$$\lambda_i(t)=\lambda_0(t)\exp(\beta_0z_i)$$
其中，$\beta_0$ 为未知的回归系数，$\lambda_0(t)$ 为“**基准风险函数**”。

该模型的核心特征是：*任意两个个体的风险函数之比是一个常数，并且不随时间* $t$ *变化*，因此称为“比例风险模型”。
